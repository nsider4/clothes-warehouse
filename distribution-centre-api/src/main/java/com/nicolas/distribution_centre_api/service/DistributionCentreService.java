...
import com.nicolas.distribution_centre_api.model.Brand;
import com.nicolas.distribution_centre_api.model.DistributionCentre;
...
public class DistributionCentreService {
    ...
    public void initData() {
        ...
        items1.add(new Item("T-Shirt", Brand.BALENCIAGA, 5, 2023, 29.99));
        items1.add(new Item("Jeans", Brand.DIOR, 4, 2022, 59.99));
        DistributionCentre centre1 = new DistributionCentre("North Hub", 43.65107, -79.347015);
        centre1.setItems(items1);

        List<Item> items2 = new ArrayList<>();
        items2.add(new Item("Jacket", Brand.GUCCI, 3, 2024, 89.99));
        DistributionCentre centre2 = new DistributionCentre("West Hub", 49.282729, -123.120738);
        centre2.setItems(items2);

        DistributionCentre centre3 = new DistributionCentre("East Hub", 45.501689, -73.567256);
        centre3.setItems(new ArrayList<>());

        List<Item> items4 = new ArrayList<>();
        items4.add(new Item("Sweater", Brand.PRADA, 40, 2023, 34.99));
        items4.add(new Item("Cap", Brand.BALENCIAGA, 25, 2021, 19.99));
        items4.add(new Item("Scarf", Brand.STONE_ISLAND, 15, 2022, 24.99));
        DistributionCentre centre4 = new DistributionCentre("South Hub", 51.044733, -114.071883);
    ...
    public Optional<Item> requestItem(String brand, String name) {
        return distributionCentreRepository.findAll().stream()
                ...
                .filter(item -> item.getBrand().name().equalsIgnoreCase(brand) && item.getName().equalsIgnoreCase(name))
                .findFirst();
    }
}
